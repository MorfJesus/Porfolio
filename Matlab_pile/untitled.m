...
for i=1:m
    ...
    if i==3
        Pmz(i)=subs(Pm, m0, i);
        Az(i)=PartSol(Pmz(i), cos(theta))*log(r);
        Qmz(i)=subs(Qm, m0, i);
        Bz(i)=PartSol(Qmz(i), sin(theta))*log(r);
        P = P + Pmz(i)*cos(i*theta) + Qmz(i)*sin(i*theta);
    end
end
...
syms A1 A2 A5 A6 A7 A8 B1 B2 B5 B6 B7 B8
...
W0_1=A0_1 + A0_2*r^2 +PartSol(P0, 1);
...
R_W0_1=A0_1 + A0_2*r^2 +PartSol(P0, 1);
...
for i=1:m
    MRR_1 = MRR_1+Mrr(R_C_1(i)*cos(i*theta))+Mrr(R_S_1(i)*sin(i*theta));
    MRR_2 = MRR_2+Mrr(R_C_2(i)*cos(i*theta))+Mrr(R_S_2(i)*sin(i*theta));
end

MRR_1=MRR_1+Mrr(R_W0_1);
MRR_2=MRR_2+Mrr(R_W0_2);
...
function [res] = UniqueSol(W0_1, W0_2, vars, fun) %ONLY WORKS FOR YOU, WILL NEED TO CHANGE ACCORDING TO CONDITIONS
    global a c b
    syms r
    eqns = [subs(W0_1*fun, r, a)==subs(W0_2*fun, r, a)
    subs(diff(W0_1*fun, r), r, a)==subs(diff(W0_2*fun, r), r, a)
    subs(Mrr(W0_1*fun), r, a)==subs(Mrr(W0_2*fun), r, a)
    subs(Qr(W0_1*fun), r, a)==subs(Qr(W0_2*fun), r, a)
    subs(W0_2*fun, r, b)==0
